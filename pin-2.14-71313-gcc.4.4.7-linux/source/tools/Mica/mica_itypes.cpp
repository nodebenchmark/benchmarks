...
VOID itypes_count(THREADID threadId, UINT32 gid){
    if (!enabled[threadId])
        return;
...
VOID instrument_itypes(INS ins, VOID* v){
				...
				if(group_identifiers[i][j].type == identifier_type::ID_TYPE_OPCODE){
                    ...
                    if(group_identifiers[i][j].type == identifier_type::ID_TYPE_SPECIAL) {
                        if (strcmp(group_identifiers[i][j].str, "mem_read") == 0 && INS_IsMemoryRead(ins) ) {
                            ...
                            categorized = true;
                            break;
                        } else if (strcmp(group_identifiers[i][j].str, "mem_write") == 0 && INS_IsMemoryWrite(ins) ) {
                            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)itypes_count, IARG_THREAD_ID, IARG_UINT32, i, IARG_END);
                            categorized = true;
                            break;
                        } else if (strcmp(group_identifiers[i][j].str, "direct") == 0 && INS_IsDirectBranchOrCall(ins) ) {
                            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)itypes_count, IARG_THREAD_ID, IARG_UINT32, i, IARG_END);
                            categorized = true;
	...
	if( !categorized ){
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)itypes_count, IARG_THREAD_ID, IARG_UINT32, (unsigned int)number_of_groups, IARG_END);
...
VOID fini_itypes(INT32 code, VOID* v){
	...
	if(interval_size == -1){
		//result_file.open(mkfilename("itypes_full_int"), ios::out|ios::trunc);
		result_file << total_ins_count;
		for(i=0; i < number_of_groups+1; i++){
		...
		//result_file.open(mkfilename("itypes_phases_int"), ios::out|ios::app);
		result_file << interval_ins_count;
		for(i=0; i < number_of_groups+1; i++){
			result_file << " " << group_counts[i];
		}
		result_file << endl;
	}
	result_file << "number of instructions: " << total_ins_count_for_hpc_alignment << endl;
	result_file.close();
