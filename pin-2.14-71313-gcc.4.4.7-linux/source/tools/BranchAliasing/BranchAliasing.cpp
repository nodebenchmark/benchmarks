...
Predictor *perEventPredictors[128];
//Predictor *predictor;
...
KNOB<string> KnobOutputFile(KNOB_MODE_WRITEONCE,  "pintool",
    "o", "aliases.out", "specify file name for MyPinTool output");
...
KNOB<BOOL>   KnobPerEvent(KNOB_MODE_WRITEONCE, "pintool",
    "per-event", "0", "dump stats and reset after each event");
...
VOID DisablePrediction(THREADID threadId)
{
    ...
    if (perEvent && !alwaysOn) {
        // Print stats and reset structures.
        *out << endl << endl;
        *out << "================================================================================" << endl;
        *out << "===                             Event Stats Dump                             ===" << endl;
        *out << "================================================================================" << endl;
        perEventPredictors[threadId]->print(out);
        perEventPredictors[threadId]->clear();
    }
}
...
VOID TakeBranch(ADDRINT addr, THREADID threadId)
{
    ...
    if (perEvent) {
        perEventPredictors[threadId]->predict(addr, &state_ptr);
        update_event = perEventPredictors[threadId]->createUpdateEvent(perEventPredictors[threadId], curTime[threadId] + updateDelay, true, state_ptr);
        queues[threadId].enqueue(update_event);
        perEventPredictors[threadId]->updateHistory(true, state_ptr);
    }
}
...
VOID FallThroughBranch(ADDRINT addr, THREADID threadId)
{
    ...
    if (perEvent) {
        perEventPredictors[threadId]->predict(addr, &state_ptr);
        update_event = perEventPredictors[threadId]->createUpdateEvent(perEventPredictors[threadId], curTime[threadId] + updateDelay, false, state_ptr);
        queues[threadId].enqueue(update_event);
        perEventPredictors[threadId]->updateHistory(false, state_ptr);
    }
}
...
VOID threadCreated(THREADID threadIndex, CONTEXT *ctxt, INT32 flags, VOID *v)
{
    ...
    if (KnobPredictorType.Value() == "local") {
        ...
        if (perEvent) {
            perEventPredictors[threadIndex] = new
                LocalPredictor(KnobLocalPredictorSize.Value(),
                        KnobLocalPredictorHistories.Value());
        }
